...
1. `-o filenames` added to the `complete` builtin call — tells readline
   candidates are filesystem paths, suppressing bash's tendency to strip the
   dotted prefix, and ensuring directories are entered rather than
   space-terminated.  Placed on the `complete` declaration rather than via
   `compopt` inside the function body so that `$?` from the completion
   subprocess is not clobbered.
