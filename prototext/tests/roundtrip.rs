...
// ── §100 MessageSet expansion tests ──────────────────────────────────────────
...
// Wire bytes for Container { name: "mixed", extensions: TestMessageSet { ... } }
// where extensions has:
//   Item { type_id: 100, message: ExtPayloadA { label: "hello", value: 42 } }
//   Item { type_id: 999, message: <unknown bytes> }   — unresolved
const CONTAINER_MIXED_WIRE: &[u8] =
    &hex_literal::<35>("0a056d69786564121a0b10641a090a0568656c6c6f102a0c0b10e7071a040807104d0c");
...
// TC-1: both extensions resolve — canonical virtual field names appear.
#[test]
fn message_set_resolved_expansion_renders_canonical_names() {
    ...
    assert!(s.contains("Item {"), "expected 'Item {{' group opener: {s}");
    assert!(s.contains("type_id:"), "expected 'type_id:' field: {s}");
    assert!(s.contains("message {"), "expected 'message {{' field: {s}");
    assert!(s.contains("100"), "expected type_id value 100: {s}");
    assert!(s.contains("200"), "expected type_id value 200: {s}");
    assert!(
        s.contains("label:"),
        "expected ExtPayloadA field 'label': {s}"
    );
    assert!(s.contains("hello"), "expected label value 'hello': {s}");
    assert!(
        s.contains("value:"),
        "expected ExtPayloadA field 'value': {s}"
    );
    assert!(s.contains("42"), "expected value 42: {s}");
    assert!(
        s.contains("flag:"),
        "expected ExtPayloadB field 'flag': {s}"
    );
    assert!(
        s.contains("count:"),
        "expected ExtPayloadB field 'count': {s}"
    );
    assert!(s.contains("99"), "expected count 99: {s}");
}

// TC-1 annotations: group annotation uses "group; Item = 1" syntax.
#[test]
fn message_set_group_annotation_syntax() {
    let schema = message_set_schema("Container");
    let text = render_as_text(
        CONTAINER_WIRE,
        Some(&schema),
    ...
    let s = String::from_utf8(text).unwrap();
    assert!(
        s.contains("group; Item = 1"),
        "expected 'group; Item = 1' annotation: {s}"
    );
    assert!(
        s.contains("int32 = 2"),
        "expected 'int32 = 2' annotation on type_id: {s}"
    );
}

// TC-3: unknown type_id — falls back to schemaless cascade, no data lost.
#[test]
fn message_set_unknown_type_id_falls_back_schemaless() {
    let schema = message_set_schema("Container");
    let text = render_as_text(
    ...
    .unwrap();
    let s = String::from_utf8(text).unwrap();
    // The known item (type_id=100) still expands.
    assert!(s.contains("label:"), "expected known item to expand: {s}");
    // The unknown item (type_id=999) renders something — no panic, no data loss.
    // We don't assert the exact fallback form, just that the output is non-empty.
    assert!(!s.is_empty(), "output must not be empty: {s}");
}

// TC-6: round-trip — decode with annotations then re-encode → byte-identical.
#[test]
fn message_set_roundtrip() {
    ...
    assert_eq!(
        wire2,
        CONTAINER_WIRE,
        "MessageSet must round-trip byte-for-byte\ndecoded text:\n{}",
