...
def _random_value(
    ...
    if ft == FieldDescriptor.TYPE_MESSAGE:
        ...
        return _generate_message(
            nested_desc, rng, depth + 1, max_depth, max_repeated, p_optional, pool
        )
...
def _generate_message(
    ...
    for oneof in desc.oneofs:
        ...
        val = _random_value(
            chosen, rng, depth, max_depth, max_repeated, p_optional, pool
        )
        if val is not None:
    ...
    for field in desc.fields:
        ...
        if label == FieldDescriptor.LABEL_REPEATED:
            count = rng.randint(0, max_repeated)
            if count == 0:
                continue
            repeated = getattr(msg, field.name)
            for _ in range(count):
                val = _random_value(
                    field, rng, depth, max_depth, max_repeated, p_optional, pool
                )
                if val is None:
                    continue
                try:
...
def generate_instance(
    ...
    *,
    ...
    msg = _generate_message(desc, rng, 0, max_depth, max_repeated, p_optional, pool)
